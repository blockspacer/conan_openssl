...
class OpenSSLConan(conan_build_helper.CMakePackage):
    name = "openssl"
    version = "1.1.1-stable" # don't forget to change _full_version below
    git_branch = "OpenSSL_1_1_1-stable"
    ...
    #def export_sources(self):
        ...
        #
        #if self.version == "1.1.1-stable":
        #    self.copy("patches/1.1.1-tvos-watchos.patch")
        #if self.version == "1.0.2u":
        #    self.copy("patches/1.0.2u-darwin-arm64.patch")
    ...
    def source(self):
        ...
        self.run('git clone --progress --branch {} --recursive --recurse-submodules {} {}'.format(self.git_branch, self.repo_url, self._source_subfolder))

    def configure(self):
        #if self._is_clangcl:
        #    # TODO: nasm  -Ox -f win64 -DNEAR -g -o crypto\aes\aesni-mb-x86_64.obj "crypto\aes\aesni-mb-x86_64.asm"
        #    # return code '0xc0000005'
        #    self.output.warn('no_asm')
    ...
    def requirements(self):
        if self._full_version < "1.1.0" and self.options.get_safe("no_zlib") == False:
            # TODO: use self.requires("zlib/1.2.12")
...
"""
    ...
    def build(self):
        ...
        #    tools.patch(**patch)
        #if self.version == "1.1.1-stable":
        #    tools.patch(patch_file = "patches/1.0.2u-darwin-arm64.patch", base_path = self._source_subfolder)
        #if self.version == "1.0.2u":
        #    tools.patch(patch_file = "patches/1.1.1-tvos-watchos.patch", base_path = self._source_subfolder)

        with tools.vcvars(self.settings, only_diff=False) if self._use_nmake else tools.no_op(): # https://github.com/conan-io/conan/issues/6577
